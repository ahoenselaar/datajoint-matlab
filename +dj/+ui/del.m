...
if ~relvar.exists
    ...
    restrictByMe = arrayfun(@(rel) ...
                    any(ismember(...
                    cellfun(@(r) relvar.schema.conn.tableToClass(r), rel.referenced,'uni',false),...
                    list)),...
                    tabs);  % restrict by all association tables, i.e. tables that make referenced to other tables
    ...
    for i=1:length(rels)
        ...
        for ix = cellfun(@(child) find(strcmp(relvar.schema.conn.tableToClass(child),list)), [rels(i).children rels(i).referencing])
            if restrictByMe(i)
